...
A close-up look on the key entities part. When clicked on each article on the right cell, a list of key entities would be displayed. A rank score is given for users to understand the importance of the key entity in the article, therefore users should focus on the ones with high scores to see who are the key players in the events each article describes.
